...
has_many :line_items, after_add: :add_item, after_add: :remove_item
belongs_to :client
	...
	def remove_item(line_item)
		 if line_item.valid? == false 
		 	self.update_attribute(:quantity, self.line_items.quantity - line_items.quantity)
		 end
	end
